...
// spare button action function ------------------------------------------------
// if button is pressed enable talk-through
// if button is released set button_state back to 1

uint8_t next_sp_button_st (uint8_t current_sp_button_st)
{

	uint8_t i;
	uint8_t temp_state;
	uint8_t cdc_conf_done;							//indicate that the codec has been configured

	temp_state = current_sp_button_st;				// latch current state
/*
	// determine if state must change
	if (temp_state)									// button thought to be released (1)
	{
		...
		if (GPIO_ReadInputDataBit (GPIOA, GPIO_Pin_2) == RESET)
		{
			...
			if (GPIO_ReadInputDataBit (GPIOA, GPIO_Pin_2) == RESET)
			{
				//The codec may have been powered down by: 	case lamInformation
				if (!DAC_EN)   //initialize codec if it's been powered down
				{
					codec_pwr_up();
				}
				// codec_talk_thru
				...
				codec_reg_write (0x74, 0x07);					// enable HS_BIAS
...
//				codec_reg_write (0x02, 0x06);					//ADC soft-ramp enable
				...
				//button was released
				//no need to change temp_state as it's already (1)
				//GPIO_ResetBits (GPIOB, GPIO_Pin_7);  // reset codec
				//codec_init ();									// initialize codec
				//volume will be configured by next lamAudioTerminalInformation
...
/*
	//After type detection - initiate load detection 
	codec_reg_write (0x00, 0x11);					// set page 0x11
	codec_reg_write (0x01, 0xFE);					//codec is powered up
	codec_reg_write (0x00, 0x1B);					// set page 0x1B
	codec_reg_write (0x74, 0x01);					//Set HSBIAS_CTRL to Hi-Z Mode
	codec_reg_write (0x00, 0x11);					// set page 0x11
	codec_reg_write (0x01, 0x04);					//Set ADPTPWR to Fixed, Mode 3
	codec_reg_write (0x00, 0x10);					// set page 0x10
	codec_reg_write (0x0A, 0x71);					//Set the analog and digital soft ramp rates
	codec_reg_write (0x00, 0x19);					// set page 0x19
	codec_reg_write (0x27, 0x01);					//Enable HP load detect
	//Read HPLOAD_DET_DONE until value is 1
	do
	{
		temp_rd = codec_reg_read  (0x26);			// read 0x1926
		if ((temp_rd & 0x01) == 0x01){		//test bit 0
			auto_done = 1;									// set flag showing it completed
		} else {
			wait_15ms ();							// delay 
		}	
	...
	codec_reg_write (0x00, 0x19);					// set page 0x19
	temp_rd = codec_reg_read (0x25);
	//Set HPOUT_LOAD according to CLA_STAT
	if (temp_rd & 0x10){		//test bit 4
		D_CTRL_2 = 0x09;									// set for 10-nF mode
	} else {
		D_CTRL_2 = 0x01;									// set for 1-nF mode
	}	
	codec_reg_write (0x00, 0x21);					// set page 0x21
	codec_reg_write (0x01, 0x07);					//Restore ADPTPWR Adapt-to-Signal Mode
	codec_reg_write (0x00, 0x1B);					// set page 0x1B
	codec_reg_write (0x74, 0x07);					//Set HSBIAS_CTRL to 2.7V Mode
	codec_reg_write (0x00, 0x11);					// set page 0x11
	codec_reg_write (0x01, 0xF6);					//Power up the HP again.
	codec_reg_write (0x00, 0x10);					// set page 0x10
	codec_reg_write (0x0A, 0xA4);					//Set the analog and digital soft ramp rates
	codec_reg_write (0x00, 0x19);					// set page 0x19
	codec_reg_write (0x27, 0x00);					//Disable HP load detection
	codec_reg_write (0x25, 0x1F);					//Clear latch to VP
*/	
